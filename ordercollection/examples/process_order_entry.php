...
$data=array($application_date, $sample_received_date, $receipt_number, $owner_name, $owner_number, $owner_email_id, $place, $address, $doctor_name, $doctor_number, $doctor_email_id, $reference_number, "0");
// prepare and bind
$stmt = $conn->prepare("INSERT INTO ORDER_ENTRY(application_date, sample_received_date, cm_receipt_number, owner_name, owner_number, owner_email_id, place, address, doctor_name, doctor_number, doctor_email_id, reference_number, order_state) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
$order_id = $ordersResult['order_id'];
	...
	for($i=0;$i<sizeof($species);$i++){
		$data=array($order_id, $barcode[$i], $species[$i], $sampleType[$i], $animalAge[$i], $animalSex[$i], $animalHistory[$i], $testsRequired[$i], $diseaseSuspect[$i]);
		$stmt = $conn->prepare("INSERT INTO SAMPLE_ENTRY(order_id, barcode,species, sample_type, animal_age, sex, animal_history, tests_required, disease_suspected) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
